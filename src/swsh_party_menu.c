...
#include "swsh_party_menu.h" // Overhaul: tuning toggles live here, not in config/swsh_party_menu.h
    ...
    else
    {
        ...
        if (tQuantityInBag > 1)
        {
            ...
            else // Exp Candies
            {
                ...
                // Overhaul: u32, not u16 -- the EXP_TO_CAP tier is 2,000,000 and would truncate to 33,920.
                u32 candyExp = sExpCandyExperienceTable[tHoldEffectParam - 1];
                u32 candyCount = (totalExp + candyExp - 1) / candyExp;
