...
// Nuzlocke: the graveyard box only takes the dead, and only via the automatic
// sweep, so it is never a legal deposit target.
//
// This is the SwSh equivalent of the TryStorePartyMonInBox() gate in
// src/pokemon_storage_system.c. That function has no counterpart here: the SwSh
// deposit flow does not hand the mon to a "store it for me" helper, it validates
// the box the player picked in the choose-box grid inline (free-slot check),
// scrolls to it and then drives the placement itself. So the gate moves to the
// validation step, which is the only point where a box can still be refused.
static bool32 CanDepositMonInBox(u8 boxId)
{
    if (NuzlockeIsGraveyardBox(boxId))
        return FALSE;

    return GetFirstFreeBoxSpot(boxId) != -1;
}
    ...
    do
    {
        ...
        if ((JOY_NEW(A_BUTTON)) && SetSelectionMenuTexts())
        {
            ...
            if (sCursorMode == CURSOR_MODE_MULTI_MOVE)
            {
                ...
                {
                    // Nuzlocke rule 7: multi-move grabs a whole rectangle, so
                    // the per-mon gate in SetMenuTexts_Mon only covers the
                    // anchor. The graveyard box refuses group grabs outright.
                    // NOTE (vs. src/pokemon_storage_system.c): vanilla starts
                    // multi-move from SELECT_BUTTON and is gated in
                    // Task_PokeStorageMain's INPUT_MULTIMOVE_START case. Here
                    // multi-move is a cursor mode and the input handler has
                    // already committed sStorage->inBoxMovingMode by the time
                    // the task sees it, so the gate must sit above that write.
                    if (NuzlockeIsGraveyardBox(StorageGetCurrentBox()))
                    {
                        PlaySE(SE_FAILURE);
                        return INPUT_NONE;
...
static bool8 SetMenuTexts_Mon(void)
{
    ...
    // Refusing here kills the A-press for every box option at once, in all
    // three cursor modes (normal menu, auto-action and multi-move) -- every
    // one of them goes through SetSelectionMenuTexts() first.
