...
// Overhaul: NUM_COMFY_ANIMS + 1 entries. The extra trailing slot is the overflow
// slot at index INVALID_COMFY_ANIM -- see CreateComfyAnim_Easing below.
EWRAM_DATA struct ComfyAnim gComfyAnims[NUM_COMFY_ANIMS + 1] = {0};
...
void AdvanceComfyAnimations(void)
{
    ...
    // Overhaul: <= so the overflow slot is advanced too.
...
{
    // Overhaul: upstream returned INVALID_COMFY_ANIM here without initialising
    // anything, and every caller then does gComfyAnims[id] -- an out-of-bounds
    // EWRAM access one struct past the pool. gComfyAnims is now sized
    // NUM_COMFY_ANIMS + 1 and the overflow slot is initialised like any other,
    // so an exhausted pool degrades to a shared animation instead of memory
    // corruption, and the caller's `completed` check still terminates.
...
{
    // Overhaul: same overflow-slot guard as CreateComfyAnim_Easing above.
    u32 i = GetAvailableComfyAnim();
...
{
    // Overhaul: <= so the overflow slot can be released as well.
