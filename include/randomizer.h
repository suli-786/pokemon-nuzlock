...
// scripts (data/scripts/obtain_item.inc); they are no-ops unless
// RANDOMIZER_AVAILABLE is TRUE and the field item flag is set.
...
#define STARTER_AND_GIFT_MON_COUNT 10 // Must match the number of entries in gStarterAndGiftMonTable.
#define EGG_MON_COUNT 1 // Must match the number of entries in gEggMonTable.

extern const u16 gStarterAndGiftMonTable[];
extern const u16 gEggMonTable[];

// How the randomizer treats a species as a randomization result.
// (Upstream stored this in gSpeciesInfo; this port derives it from existing
// species flags plus explicit lists in src/data/randomizer/species_modes.h.)
enum RandomizerSpeciesFormMode {
...
// Given an egg slot and the list of original egg mons, returns the random egg mon in that slot.
