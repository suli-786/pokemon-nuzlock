...
// Overhaul: raised from upstream's 8. Eight slots is barely one screen's worth --
// the SwSh summary screen alone holds several while the map pop-up keeps one
// across map transitions.
#define NUM_COMFY_ANIMS     16
#define INVALID_COMFY_ANIM  NUM_COMFY_ANIMS
// Overhaul: one slot past the pool. CreateComfyAnim_* returns INVALID_COMFY_ANIM
// when the pool is exhausted and every caller indexes gComfyAnims[id] unguarded,
// so the array is sized to keep that index in bounds. See comfy_anim.c.
extern struct ComfyAnim gComfyAnims[NUM_COMFY_ANIMS + 1];
