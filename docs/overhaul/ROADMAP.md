...
#### 7.12 status — Phase 4 step 1 (habitat layer) LANDED 2026-07-29

Rule 1's data layer is built and guarded; nothing else in 7.12 is implemented yet. Encounters are unchanged — step 2's table-level generator is the first thing that consumes any of this.

- **Eleven habitats**, `include/constants/habitat.h`, as bit sets on both sides. A candidate is legal for a slot when `GetSpeciesHabitats() & GetAreaHabitats()` is non-zero. Sets, not single values: Golbat is cave *and* mountain, a mountain path is mountain *and* field, and that overlap is what stops low-BST bands starving.
- **All 116 Hoenn encounter maps tagged** in `src/data/habitats.h` — matching GYM_CLAIMS.md's verified count exactly. Land also seeds rock smash (plus `HABITAT_CAVE`); water also covers fishing. Kanto/Sevii maps are deliberately absent and answer `HABITAT_NONE`.
- **Classifier is three layers, unioned** (`src/habitat.c`): (1) where Emerald itself places the species, generated into `src/data/habitats_vanilla.h`; (2) type and egg-group rules; (3) a manual override list, currently 3 entries.
- **Layer 1 deviates from the spec, deliberately.** The spec named "official Gen1-3 habitat data", which does not exist in this repo — the expansion has no habitat field, and FRLG's Pokédex habitat covers Kanto's 151 only. The game's own encounter tables are a strictly better source: in-repo, self-consistent with the very maps being tagged, and authoritative about the species the game was balanced around. It is also load-bearing, not cosmetic: the type rules are geological (Rock/Ground/Steel/Mineral), so on their own they put **Zubat on mountains and Whismur and Makuhita in fields** — all three are Granite Cave and Rusturf Tunnel residents. Caught by a test, not by inspection.
- **Layer 2 is derived, not tabulated.** ~20 rules classify 1025 of the 1028 randomizer-eligible species. A hand-authored table would be 1028 rows restating the rules and would rot the moment a typing changed. The 3 stragglers are Cleffa, Igglybuff and Togepi — babies whose egg group is `NO_EGGS_DISCOVERED`.
- **No freshwater habitat.** Prototyped and dropped: thinnest bucket by far (37 species) and not in the spec's eleven. Ponds and rivers are surface water.
- **Guards** (`test/habitat.c`): every species has ≥1 habitat (the completeness guarantee); no species is missing the habitat Emerald actually puts it in, re-derived at runtime from the live tables so a stale generated file fails the build rather than shipping; every tagged map answers for the areas it ships tables for; spot-checks on authored tags.
- **Regenerate with** `python3 tools/overhaul/gen_habitats.py` after editing `wild_encounters.json` or the map tags.

**Measured, for step 2's benefit:** habitat ∩ ±10% BST is very thin at the bottom of the curve — a cave slot near 220 BST has 4 candidates, ice near 240 has 1, desert near 220 has 0. Weak Pokémon are overwhelmingly field and forest animals while cave, ice and desert species skew strong. Two consequences, both agreed with the owner: the BST band is **±10% or ±40 points, whichever is wider** (±10% of 200 is only ±20), and the area-diversity rule means **preserve each table's vanilla distinct-species count and make those targets pairwise distinct** — *not* 12 distinct species per land table. Vanilla land tables average 4.1 distinct species across 12 slots and 60 of them use just one; requiring 12 would be infeasible for several habitats and would erase the "this route is mostly Zigzagoon" texture.
